...
RESULT_ROOT = REPO_ROOT.parent / "results" / "final_mujoco_state_actor_coupling_positive_scorefix"
...
RHO_GRID = [0.5, 1.0, 2.0, 5.0]
LAMBDA_U_GRID = [0.01, 0.05]
LAMBDA_W_GRID = [0.01, 0.05]
...
def collect_state_dataset(spec: EnvSpec, seed: int, target_size: int) -> tuple[torch.Tensor, torch.Tensor]:
    ...
    rng.shuffle(arr)
    split = int(TRAIN_FRACTION * len(arr))
    train = torch.as_tensor(arr[:split], dtype=DTYPE, device=DEVICE)
    eval_states = torch.as_tensor(arr[split:], dtype=DTYPE, device=DEVICE)
...
def assess_config(curves_by_method: dict[str, list[dict[str, Any]]], summaries: dict[str, dict[str, Any]]) -> dict[str, Any]:
    ...
    dom_qp_egm = dominance_fraction(curves_by_method["proposed_qp_nog_safe"], curves_by_method["egm"], "actor_game_score")
    dom_qp_nog = dominance_fraction(curves_by_method["proposed_qp_nog_safe"], curves_by_method["proposed_nog_closed"], "actor_game_score")
    final_best = max(summaries.values(), key=lambda row: safe_float(row["final_actor_game_score"]))["method"]
    decision = "NO_CLEAN_GAME_QP_POSITIVE"
    if (
        qp_vs_egm >= 0.10
        and qp_vs_nog >= 0.05
        ...
        and qp["field_norm_AUC"] <= min(egm["field_norm_AUC"], nog["field_norm_AUC"]) + 1e-9
        and qp["curve_sanity_flag"] == 1
    ):
        decision = "QP_CLEAN_GAME_WEAK_POSITIVE"
    if (
        qp_vs_egm >= 0.15
        and qp_vs_nog >= 0.15
        ...
        and qp["QP_better_than_noG_actual_V_fraction"] >= 0.80
        and final_best == "proposed_qp_nog_safe"
        and qp["final_field_norm"] <= min(egm["final_field_norm"], nog["final_field_norm"], sgd["final_field_norm"]) + 1e-9
        and qp["curve_sanity_flag"] == 1
    ):
        decision = "QP_CLEAN_GAME_STRONG_POSITIVE"
    return {
        "decision": decision,
...
def run_training_for_configs(
    ...
    if not passing:
        return [], [], "NO_CLEAN_GAME_QP_POSITIVE"

    all_curves: list[dict[str, Any]] = []
    ranked_rows: list[dict[str, Any]] = []
    report_lines = ["# positive training report", ""]
    ...
    for gate in passing:
        ...
        rank_row = {**gate, **assess, **summaries["proposed_qp_nog_safe"], "config_slug": cfg.slug}
        ranked_rows.append(rank_row)
        report_lines.append(
            f"- `{cfg.slug}`: decision=`{assess['decision']}`, qp_vs_egm=`{assess['qp_vs_egm_auc_frac']:.3f}`, "
            f"qp_vs_nog=`{assess['qp_vs_nog_auc_frac']:.3f}`, egm_vs_sgd=`{assess['egm_vs_sgd_auc_frac']:.3f}`, "
            f"fallback=`{summaries['proposed_qp_nog_safe']['fallback_to_noG_frac']:.3f}`"
        )
    ...
    ranked_rows.sort(
        key=lambda row: (
            {"QP_CLEAN_GAME_STRONG_POSITIVE": 0, "QP_CLEAN_GAME_WEAK_POSITIVE": 1, "NO_CLEAN_GAME_QP_POSITIVE": 2}.get(str(row["decision"]), 3),
            -safe_float(row["qp_vs_egm_auc_frac"]),
            -safe_float(row["payoff_J_AUC"]),
        )
    )
    write_csv(out_root / "02_positive_training" / "positive_training_all.csv", all_curves)
    write_csv(out_root / "02_positive_training" / "positive_training_ranked.csv", ranked_rows)
    write_text(out_root / "02_positive_training" / "positive_training_report.md", "\n".join(report_lines) + "\n")
    top_lines = ["# positive top configs", ""]
    for idx, row in enumerate(ranked_rows[:10], start=1):
        top_lines.append(
            f"{idx}. `{row['config_slug']}` | decision=`{row['decision']}` | actor_game_score_AUC=`{safe_float(row['actor_game_score_AUC']):.6e}` | "
            f"qp_vs_egm=`{safe_float(row['qp_vs_egm_auc_frac']):.3f}` | fallback=`{safe_float(row['fallback_to_noG_frac']):.3f}`"
        )
    write_text(out_root / "02_positive_training" / "positive_top_configs.md", "\n".join(top_lines) + "\n")
    if ranked_rows:
        best_slug = str(ranked_rows[0]["config_slug"])
        save_training_plot(out_root / "plots" / "scorefix_top_curves.png", best_slug, curves_cache[best_slug])
    final_decision = str(ranked_rows[0]["decision"]) if ranked_rows else "NO_CLEAN_GAME_QP_POSITIVE"
...
def run_multiseed_confirmation(best_row: dict[str, Any], datasets: dict[str, tuple[EnvSpec, torch.Tensor, torch.Tensor]], out_root: Path) -> str:
    if str(best_row["decision"]) not in {"QP_CLEAN_GAME_WEAK_POSITIVE", "QP_CLEAN_GAME_STRONG_POSITIVE"}:
        return "CLEAN_ACTOR_GAME_QP_FAILS"
    ...
    for seed in [0, 1, 2]:
        ...
        for method in CONFIRM_METHODS:
            ...
            curves_by_method[method] = curves
            summaries[method] = summary
        assess = assess_config({**curves_by_method, "proposed_qp_closed": curves_by_method["proposed_qp_nog_safe"]}, {**summaries, "proposed_qp_closed": summaries["proposed_qp_nog_safe"]})
        for method, summary in summaries.items():
            rows.append({"seed": seed, "method": method, "config_slug": cfg.slug, **summary, **assess})
    write_csv(out_root / "03_multiseed_confirmation" / "multiseed_summary.csv", rows)
    by_method: dict[str, list[dict[str, Any]]] = {}
    for row in rows:
        by_method.setdefault(str(row["method"]), []).append(row)
    qp_means = float(np.mean([safe_float(row["payoff_J_AUC"]) for row in by_method.get("proposed_qp_nog_safe", [])]))
    baseline_best = max(
        float(np.mean([safe_float(row["payoff_J_AUC"]) for row in by_method.get("sgd_gda", [])])),
        float(np.mean([safe_float(row["payoff_J_AUC"]) for row in by_method.get("egm", [])])),
        float(np.mean([safe_float(row["payoff_J_AUC"]) for row in by_method.get("proposed_nog_closed", [])])),
    )
    qp_best_count = 0
    fallback_mean = float(np.mean([safe_float(row["fallback_to_noG_frac"]) for row in by_method.get("proposed_qp_nog_safe", [])]))
    for seed in [0, 1, 2]:
        seed_rows = [row for row in rows if int(row["seed"]) == seed]
        best = max(seed_rows, key=lambda row: safe_float(row["payoff_J_AUC"]))
        qp_best_count += int(str(best["method"]) == "proposed_qp_nog_safe")
    decision = "CLEAN_ACTOR_GAME_QP_ONE_SEED_ONLY"
    if qp_means > baseline_best and qp_best_count >= 2 and fallback_mean <= 0.30:
        decision = "CLEAN_ACTOR_GAME_QP_CONFIRMED"
    report = [
        "# multiseed confirmation",
        "",
        f"- config_slug: `{cfg.slug}`",
        f"- qp_mean_payoff_J_AUC: `{qp_means:.6e}`",
        f"- baseline_best_mean_payoff_J_AUC: `{baseline_best:.6e}`",
    ...
    write_text(out_root / "03_multiseed_confirmation" / "multiseed_report.md", "\n".join(report))
    return decision
...
def main() -> None:
    ...
    ensure_dir(RESULT_ROOT / "00_invariant_checks")
    ensure_dir(RESULT_ROOT / "01_geometry_gate")
    ensure_dir(RESULT_ROOT / "02_positive_training")
    ensure_dir(RESULT_ROOT / "03_multiseed_confirmation")
    ...
    for env_id in ENV_ORDER:
        ...
        if spec is None:
            continue
        train_states, eval_states = collect_state_dataset(spec, SEED, DATASET_SIZE)
        datasets[env_id] = (spec, train_states, eval_states)
        available_specs.append(spec)
    if not available_specs:
        write_text(RESULT_ROOT / "mujoco_state_actor_coupling_scorefix_decision.md", "NO_USABLE_ACTOR_GEOMETRY_EVEN_IN_CLEAN_GAME\n")
        write_text(RESULT_ROOT / "mujoco_state_actor_coupling_scorefix_report.md", "No MuJoCo env was available.\n")
        return

    invariant_spec = available_specs[0]
    invariant_decision, _, invariant_text = run_invariant_check(invariant_spec, datasets[invariant_spec.env_id][1], datasets[invariant_spec.env_id][2], RESULT_ROOT / "00_invariant_checks")
    if invariant_decision != "QP_INVARIANT_PASS":
        write_text(RESULT_ROOT / "mujoco_state_actor_coupling_scorefix_decision.md", "QP_INVARIANT_FAIL\n")
        write_text(RESULT_ROOT / "mujoco_state_actor_coupling_scorefix_report.md", invariant_text)
        return
    ...
    leakage_decision, _, leakage_text = run_score_leakage_check(
        ...
        RESULT_ROOT / "00_invariant_checks",
    )
    if leakage_decision != "ACTOR_GAME_SCORE_LEAKAGE_PASS":
        write_text(RESULT_ROOT / "mujoco_state_actor_coupling_scorefix_decision.md", "ACTOR_GAME_SCORE_LEAKAGE_BUG\n")
        write_text(RESULT_ROOT / "mujoco_state_actor_coupling_scorefix_report.md", leakage_text)
        return

    gate_rows: list[dict[str, Any]] = []
    gate_lines = ["# geometry gate report", ""]
    total_passers = 0
    for spec in available_specs:
        ...
        for rho in RHO_GRID:
            for lambda_u in LAMBDA_U_GRID:
                for lambda_w in LAMBDA_W_GRID:
                    for lr in LR_GRID:
                        cfg = Config(spec.env_id, rho, lambda_u, lambda_w, lr, LAMBDA_F, LAMBDA_J)
                        row = geometry_gate_for_config(spec, cfg, train_states, eval_states)
                        env_rows.append(row)
                        gate_rows.append(row)
        env_pass = [row for row in env_rows if int(row["geometry_gate_pass"]) == 1]
        total_passers += len(env_pass)
        ...
        gate_lines.append("")
        if total_passers >= GATE_TOPK:
            break

    write_csv(RESULT_ROOT / "01_geometry_gate" / "geometry_gate_summary.csv", gate_rows)
    write_text(RESULT_ROOT / "01_geometry_gate" / "geometry_gate_report.md", "\n".join(gate_lines) + "\n")

    passing = [row for row in gate_rows if int(row["geometry_gate_pass"]) == 1]
    if not passing:
        report = [
            "# MuJoCo-state actor coupling report",
            "",
            "This benchmark uses MuJoCo observation/action spaces but replaces environment reward with a differentiable actor-coupling objective.",
            "",
            "## Final decision",
            "- `NO_USABLE_ACTOR_GEOMETRY_EVEN_IN_CLEAN_GAME`",
            "",
        ]
        write_csv(RESULT_ROOT / "mujoco_state_actor_coupling_scorefix_ranked.csv", gate_rows)
        write_text(RESULT_ROOT / "mujoco_state_actor_coupling_scorefix_decision.md", "NO_USABLE_ACTOR_GEOMETRY_EVEN_IN_CLEAN_GAME\n")
        write_text(RESULT_ROOT / "mujoco_state_actor_coupling_scorefix_report.md", "\n".join(report))
        write_text(RESULT_ROOT / "mujoco_state_actor_coupling_scorefix_top_configs.md", "# top configs\n\nNo gate-passing config.\n")
        return
    ...
    report_lines = [
        "# MuJoCo-state differentiable actor-coupling report",
        "",
    ...
    best_row = training_ranked[0] if training_ranked else sorted(passing, key=lambda row: safe_float(row["gate_score"]), reverse=True)[0]
    for key in ["config_slug", "env_id", "rho", "lambda_u", "lambda_w", "joint_lr", "cross_to_same_ratio", "rotation_ratio_proxy", "gamma_active_frac", "fallback_to_noG_frac", "actor_game_score_AUC", "pure_env_return_AUC"]:
        if key in best_row:
    ...
    write_csv(RESULT_ROOT / "mujoco_state_actor_coupling_scorefix_ranked.csv", training_ranked if training_ranked else passing)
    top_lines = ["# MuJoCo state actor coupling top configs", ""]
    source_rows = training_ranked if training_ranked else sorted(passing, key=lambda row: safe_float(row["gate_score"]), reverse=True)
    ...
    write_text(RESULT_ROOT / "mujoco_state_actor_coupling_scorefix_top_configs.md", "\n".join(top_lines) + "\n")
    write_text(RESULT_ROOT / "mujoco_state_actor_coupling_scorefix_report.md", "\n".join(report_lines))
    write_text(RESULT_ROOT / "mujoco_state_actor_coupling_scorefix_decision.md", final_decision + "\n")
