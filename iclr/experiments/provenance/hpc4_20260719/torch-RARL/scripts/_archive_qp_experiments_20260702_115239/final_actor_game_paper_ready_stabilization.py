...
RESULT_ROOT = REPO_ROOT.parent / "results" / "final_actor_game_paper_ready_stabilization"
...
def run_nog_audit(spec: EnvSpec, train_states: torch.Tensor, eval_states: torch.Tensor, out_root: Path) -> tuple[str, list[dict[str, Any]], str]:
    cfg = Config(spec.env_id, 5.0, 0.01, 0.05, 3e-4, 0.0003, 1.0, batch_size=2048)
    game = MujocoStateActorGame(spec, cfg, train_states, eval_states, SEED)
    z = game.init_z(seed_offset=SEED)
    game.metric_refs = {}
    rows: list[dict[str, Any]] = []
    ...
    decision = "NOG_AUDIT_PASS"
    if not (
        nonzero_param_frac > 0.5
        and beta_active_frac > 0.3
        and actor_delta_frac > 0.5
    ):
        decision = "NOG_BASELINE_FROZEN_OR_TOO_CONSERVATIVE"
    report = [
        "# noG audit report",
        "",
        f"- env: `{spec.env_id}`",
    ...
    write_csv(out_root / "nog_audit_steps.csv", rows)
    write_text(out_root / "nog_audit_report.md", "\n".join(report))
    return decision, rows, "\n".join(report)
...
def save_acceptance_bar_plot(plot_path: Path, summary_rows: list[dict[str, Any]]) -> None:
    ...
    values = [
        ...
        float(np.mean([safe_float(r["effective_G_contribution_ratio"]) for r in qp_rows])),
    ]
...
def main() -> None:
    ...
    if not available_specs:
        write_text(RESULT_ROOT / "paper_ready_decision.md", "DAMPED_QP_INCONCLUSIVE\n")
        write_text(RESULT_ROOT / "paper_ready_report.md", "No MuJoCo env was available.\n")
        return
    ...
    if invariant_decision != "QP_INVARIANT_PASS":
        write_text(RESULT_ROOT / "paper_ready_decision.md", "QP_INVARIANT_FAIL\n")
        write_text(RESULT_ROOT / "paper_ready_report.md", invariant_text)
        return
    ...
    if leakage_decision != "ACTOR_GAME_SCORE_LEAKAGE_PASS":
        write_text(RESULT_ROOT / "paper_ready_decision.md", "ACTOR_GAME_SCORE_LEAKAGE_BUG\n")
        write_text(RESULT_ROOT / "paper_ready_report.md", leakage_text)
        return

    nog_decision, _, nog_text = run_nog_audit(invariant_spec, datasets[invariant_spec.env_id][1], datasets[invariant_spec.env_id][2], RESULT_ROOT)
    if nog_decision != "NOG_AUDIT_PASS":
        write_text(RESULT_ROOT / "paper_ready_decision.md", "NOG_BASELINE_FROZEN_OR_TOO_CONSERVATIVE\n")
        write_text(RESULT_ROOT / "paper_ready_report.md", nog_text)
        return
                    ...
                    for lr in LR_GRID:
                        for batch_size in BATCH_SIZE_GRID:
                            for lambda_F_value, lambda_J_value in MERIT_WEIGHT_CONFIGS:
                                cfg = Config(spec.env_id, rho, lambda_u, lambda_w, lr, lambda_F_value, lambda_J_value, batch_size=batch_size)
    ...
    if training_ranked:
        final_decision = run_multiseed_confirmation(training_ranked[0], datasets, RESULT_ROOT)
    ...
    write_csv(RESULT_ROOT / "paper_ready_ranked.csv", training_ranked if training_ranked else gate_rows)
    top_lines = ["# paper ready top configs", ""]
    ...
    write_text(RESULT_ROOT / "paper_ready_top_configs.md", "\n".join(top_lines) + "\n")
    write_text(RESULT_ROOT / "paper_ready_report.md", "\n".join(report_lines))
    write_text(RESULT_ROOT / "paper_ready_decision.md", final_decision + "\n")
