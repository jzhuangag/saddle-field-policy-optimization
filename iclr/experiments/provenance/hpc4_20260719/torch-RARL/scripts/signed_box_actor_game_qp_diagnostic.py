...
RESULT_ROOT = WORK_ROOT / "results" / "signed_box_actor_game_qp_diagnostic"
...
def main() -> None:
    ...
    ensure_dir(RESULT_ROOT / "00_sign_diagnostic")
    ...
    if spec is None:
        write_text(RESULT_ROOT / "signed_box_decision.md", "SIGNED_BOX_FAILS\n")
        write_text(RESULT_ROOT / "signed_box_report.md", "No MuJoCo env was available.\n")
        return
    train_states, eval_states = collect_state_dataset(spec, SEED, TRAIN_DATASET_SIZE, EVAL_DATASET_SIZE)

    all_sign_rows: list[dict[str, Any]] = []
    ranked_rows: list[dict[str, Any]] = []
    method_rows: list[dict[str, Any]] = []
    best_curves: dict[str, list[dict[str, Any]]] | None = None
    best_rank: dict[str, Any] | None = None

    for lr in LR_GRID:
        cfg = Config(spec.env_id, RHO_GRID[0], LAMBDA_U_GRID[0], LAMBDA_W_GRID[0], lr, LAMBDA_F, LAMBDA_J, batch_size=BATCH_SIZE_GRID[0])
        sign_decision, sign_rows, sign_text = run_sign_diagnostic(spec, cfg, train_states, eval_states, RESULT_ROOT)
        all_sign_rows.extend(sign_rows)
        write_text(RESULT_ROOT / "00_sign_diagnostic" / f"{cfg.slug}_sign_diagnostic_report.md", sign_text + "\n")
        inclusion_pass_frac = run_signed_inclusion_check(spec, cfg, train_states, eval_states)
        game = MujocoStateActorGame(spec, cfg, train_states, eval_states, SEED)
        curves_by_method: dict[str, list[dict[str, Any]]] = {}
        summaries: dict[str, dict[str, Any]] = {}
        for method in METHODS:
        ...
        rank_row = assess_signed_config(cfg, summaries, curves_by_method, sign_decision, inclusion_pass_frac)
        ranked_rows.append(rank_row)
        rank_key = (
            {"SIGNED_BOX_DIAGNOSTIC_POSITIVE": 0, "SIGNED_QP_POSITIVE_BUT_EGM_NOT_ORDERED": 1, "FIELD_SIGN_MISMATCH_LIKELY": 2, "EGM_NOT_A_VALID_MIDDLE_BASELINE": 3, "SIGNED_BOX_FAILS": 4}.get(str(rank_row["decision"]), 5),
            -safe_float(rank_row["qp_signed_actor_game_score_AUC"]),
        )
        best_key = (
            {"SIGNED_BOX_DIAGNOSTIC_POSITIVE": 0, "SIGNED_QP_POSITIVE_BUT_EGM_NOT_ORDERED": 1, "FIELD_SIGN_MISMATCH_LIKELY": 2, "EGM_NOT_A_VALID_MIDDLE_BASELINE": 3, "SIGNED_BOX_FAILS": 4}.get(str(best_rank["decision"]), 5),
            -safe_float(best_rank["qp_signed_actor_game_score_AUC"]),
        ) if best_rank is not None else None
        if best_rank is None or rank_key < best_key:
            best_rank = rank_row
            best_curves = curves_by_method

    ranked_rows.sort(
        key=lambda row: (
            {"SIGNED_BOX_DIAGNOSTIC_POSITIVE": 0, "SIGNED_QP_POSITIVE_BUT_EGM_NOT_ORDERED": 1, "FIELD_SIGN_MISMATCH_LIKELY": 2, "EGM_NOT_A_VALID_MIDDLE_BASELINE": 3, "SIGNED_BOX_FAILS": 4}.get(str(row["decision"]), 5),
            -safe_float(row["qp_signed_actor_game_score_AUC"]),
        )
    )
    final_decision = str(ranked_rows[0]["decision"]) if ranked_rows else "SIGNED_BOX_FAILS"

    write_csv(RESULT_ROOT / "00_sign_diagnostic" / "sign_diagnostic_summary.csv", all_sign_rows)
    sign_report_lines = ["# aggregate sign diagnostic", ""]
    for row in ranked_rows:
        sign_report_lines.append(
            f"- `{row['config_slug']}`: sign_decision=`{row['sign_decision']}`, inclusion=`{safe_float(row['signed_qp_predicted_inclusion_pass_frac']):.3f}`"
        )
    write_text(RESULT_ROOT / "00_sign_diagnostic" / "sign_diagnostic_report.md", "\n".join(sign_report_lines) + "\n")
    write_csv(RESULT_ROOT / "signed_box_method_summary.csv", method_rows)
    write_csv(RESULT_ROOT / "signed_box_ranked.csv", ranked_rows)

    if best_curves is not None:
        save_metric_plot(RESULT_ROOT / "plots" / "signed_box_actor_game_score.png", best_curves, "actor_game_score", "Signed-box Actor Game Score (raw)", smoothed=False)
        save_metric_plot(RESULT_ROOT / "plots" / "signed_box_actor_game_score_ema.png", best_curves, "actor_game_score", "Signed-box Actor Game Score (EMA)", smoothed=True)
        save_metric_plot(RESULT_ROOT / "plots" / "signed_box_field_norm.png", best_curves, "field_norm", "Signed-box Field Norm", smoothed=True)
    save_sign_distribution_plot(RESULT_ROOT / "plots" / "beta_gamma_sign_distribution.png", ranked_rows)
    save_signed_qp_acceptance_plot(RESULT_ROOT / "plots" / "signed_qp_acceptance.png", ranked_rows)

    report_lines = [
        "# Signed-box actor-game QP diagnostic",
        "",
        f"- env: `{spec.env_id}`",
        f"- seed: `{SEED}`",
        f"- D_train: `{TRAIN_DATASET_SIZE}`",
        f"- D_eval: `{EVAL_DATASET_SIZE}`",
    ...
    for row in ranked_rows:
        report_lines.append(
            f"- `{row['config_slug']}`: decision=`{row['decision']}`, qp_vs_egm=`{safe_float(row['qp_vs_egm_auc_frac']):.3f}`, qp_vs_signed_noG=`{safe_float(row['qp_vs_signed_nog_auc_frac']):.3f}`, signed_noG_vs_sgd=`{safe_float(row['signed_nog_vs_sgd_auc_frac']):.3f}`, egm_vs_sgd=`{safe_float(row['egm_vs_sgd_auc_frac']):.3f}`, dom_qp_egm=`{safe_float(row['qp_dom_egm']):.3f}`, dom_qp_signed_noG=`{safe_float(row['qp_dom_signed_nog']):.3f}`, signed_qp_accept=`{safe_float(row['signed_qp_accept_frac']):.3f}`, inclusion=`{safe_float(row['signed_qp_predicted_inclusion_pass_frac']):.3f}`"
        )
    report_lines.extend(["", "## Final decision", "", f"`{final_decision}`", ""])
    write_text(RESULT_ROOT / "signed_box_report.md", "\n".join(report_lines) + "\n")
    write_text(RESULT_ROOT / "signed_box_decision.md", final_decision + "\n")
