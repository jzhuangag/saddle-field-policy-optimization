...
RESULT_ROOT = WORK_ROOT / "results" / "paper_ready_actor_game_qp_final"
...
def main() -> None:
    ...
    ensure_dir(RESULT_ROOT / "01_nog_audit")
    ensure_dir(RESULT_ROOT / "02_setting_selection")
    ...
    if not available_specs:
        write_text(RESULT_ROOT / "paper_ready_final_decision.md", "QP_INVARIANT_FAIL\n")
        write_text(RESULT_ROOT / "paper_ready_final_report.md", "No MuJoCo env was available.\n")
        return

    spec = available_specs[0]
    train_states = datasets[spec.env_id][1]
    eval_states = datasets[spec.env_id][2]

    invariant_decision, _, invariant_text = run_invariant_check(spec, train_states, eval_states, RESULT_ROOT / "00_invariants")
    if invariant_decision != "QP_INVARIANT_PASS":
        write_text(RESULT_ROOT / "paper_ready_final_decision.md", "QP_INVARIANT_FAIL\n")
        write_text(RESULT_ROOT / "paper_ready_final_report.md", invariant_text)
        return

    nog_decision, _, nog_text = run_nog_audit(spec, train_states, eval_states, RESULT_ROOT)
    if nog_decision == "NOG_AUDIT_FAIL_APPLY_BUG":
        write_text(RESULT_ROOT / "paper_ready_final_decision.md", "NOG_AUDIT_FAIL_APPLY_BUG\n")
        write_text(RESULT_ROOT / "paper_ready_final_report.md", nog_text)
        return

    batch_size = BATCH_SIZE_GRID[0]
    try:
        best_setting_row, setting_ranked, setting_curve_rows = run_setting_selection(spec, train_states, eval_states, batch_size, RESULT_ROOT, nog_decision)
    except RuntimeError as exc:
        if "out of memory" not in str(exc).lower():
            raise
        batch_size = SETTING_BATCH_FALLBACK
        best_setting_row, setting_ranked, setting_curve_rows = run_setting_selection(spec, train_states, eval_states, batch_size, RESULT_ROOT, nog_decision)

    selected_cfg = Config(
        env_id=best_setting_row["env_id"],
        rho=float(best_setting_row["rho"]),
        lambda_u=float(best_setting_row["lambda_u"]),
        lambda_w=float(best_setting_row["lambda_w"]),
        lr=float(best_setting_row["joint_lr"]),
        lambda_F=float(best_setting_row["lambda_F"]),
        lambda_J=float(best_setting_row["lambda_J"]),
        batch_size=int(best_setting_row["batch_size"]),
    )
    final_decision, curve_rows, summary_rows, final_report, auc_rows = run_final_multiseed(selected_cfg, spec, train_states, eval_states, RESULT_ROOT, nog_decision)

    write_csv(RESULT_ROOT / "paper_ready_seed_level_results.csv", summary_rows)
    write_csv(RESULT_ROOT / "paper_ready_multiseed_summary.csv", curve_rows)
    write_csv(RESULT_ROOT / "paper_ready_method_auc_table.csv", auc_rows)
    write_text(RESULT_ROOT / "paper_ready_final_report.md", final_report + "\n")
    write_text(RESULT_ROOT / "paper_ready_final_decision.md", final_decision + "\n")
    write_text(
        RESULT_ROOT / "paper_ready_plot_readme.md",
        "\n".join(
            [
                "# Plot README",
                "",
                "- `actor_game_score_mean_std_raw.png`: raw fixed-eval mean ± std across seeds",
                "- `actor_game_score_mean_std_ema.png`: EMA-smoothed fixed-eval mean ± std across seeds",
                "- `rotational_coupling_mean_std_ema.png`: EMA-smoothed rotational coupling",
                "- `field_norm_mean_std_ema.png`: EMA-smoothed field norm",
                "- `lyapunov_merit_mean_std_ema.png`: EMA-smoothed Lyapunov merit",
                "- `auc_bar_actor_game_score.png`: actor-game-score AUC summary by method",
                "- `curvature_acceptance_bar.png`: QP curvature acceptance diagnostics",
                "- `boundary_noG_behavior_bar.png`: boundary noG versus curvature rejection behavior",
                "- `eta_distribution_qp.png`: chosen eta distribution for QP",
                "",
            ]
        ) + "\n",
    )
    write_text(
        RESULT_ROOT / "README_FINAL_EXPERIMENT.md",
        "\n".join(
            [
                "# Canonical Final Experiment",
                "",
                "1. This is the only canonical final experiment.",
                "2. Old experimental folders are archived.",
                "3. noG gamma/beta boundary behavior is valid field-only stalling, not an implementation bug.",
                "4. QP contains noG at the surrogate level.",
                "5. QP uses curvature only when accepted by actual frozen-batch Lyapunov.",
                "6. This benchmark is MuJoCo-state actor-coupling, not standard RARL.",
                "",
