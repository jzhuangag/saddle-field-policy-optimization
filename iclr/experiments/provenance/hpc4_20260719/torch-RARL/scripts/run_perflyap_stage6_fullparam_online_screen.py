...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser("Stage 6 full-param return+cost short online screen")
...
def build_candidates() -> List[Candidate]:
    ...
    return [
        Candidate(
            ...
            1.0,
            1.0,
            {**common, "cost_mode": "mixed_clean_unclipped_actor_surrogate_cost", "selector_mode": "fixed_nog", "fixed_beta_raw": 0.015, "fixed_gamma_raw": 0.0},
        ...
        Candidate(
            ...
            1.0,
            1.0,
            {**common, "cost_mode": "mixed_clean_unclipped_actor_surrogate_cost", "selector_mode": "actual_mixed_selector", "fixed_beta_raw": 0.009, "fixed_gamma_raw": 3e-05},
            "mixed_clean_unclipped_actor_surrogate_cost",
        ...
        Candidate(
            ...
            1.0,
            1.0,
            {**common, "cost_mode": "mixed_clean_unclipped_actor_surrogate_cost", "selector_mode": "safe_fixed_minusg", "fixed_beta_raw": 0.012, "fixed_gamma_raw": 2.4e-05},
            "mixed_clean_unclipped_actor_surrogate_cost",
            "safe_fixed_minusg",
        ),
        Candidate(
            ...
            {
                **common,
                "cost_mode": "mixed_rarl_unclipped_actor_surrogate_cost",
                "selector_mode": "actual_mixed_selector",
                "direction_mode": "performance_grad",
                "fixed_beta_raw": 0.006224178546127739,
                "fixed_gamma_raw": 2.0747261820425793e-05,
            },
...
def main() -> None:
    ...
    training_df.to_csv(output_root / "stage6_fullparam_training_curves.csv", index=False)
    eval_df.to_csv(output_root / "stage6_fullparam_eval_curves.csv", index=False)
    param_df.to_csv(output_root / "stage6_fullparam_param_norms.csv", index=False)
    qp_diag_df.to_csv(output_root / "stage6_fullparam_qp_diagnostics.csv", index=False)
    update_df.to_csv(output_root / "stage6_fullparam_block_update_audit.csv", index=False)
    summary_df.to_csv(output_root / "stage6_fullparam_online_summary.csv", index=False)
    ...
    if not qp_diag_df.empty:
        ...
        ax.legend(fontsize=8, ncol=2)
        fig.tight_layout()
        fig.savefig(plots_dir / "stage6_fallback_rate_aux.png", dpi=180, bbox_inches="tight")
        plt.close(fig)
    else:
        (plots_dir / "stage6_beta_gamma_fallback.png").write_text("", encoding="utf-8")
        (plots_dir / "stage6_fallback_rate_aux.png").write_text("", encoding="utf-8")

    if not qp_diag_df.empty:
        fig, ax = plt.subplots(figsize=(12, 7))
        for method, group in qp_diag_df[qp_diag_df["optimizer_role"] == "protagonist"].groupby("method"):
            group = group.sort_values("outer_iteration")
            ax.plot(group["outer_iteration"], pd.to_numeric(group["mixed_merit_change"], errors="coerce"), label=f"{method} mixed")
            ax.plot(group["outer_iteration"], pd.to_numeric(group["short_clean_return_cost_change"], errors="coerce"), linestyle="--", label=f"{method} short_clean")
            ax.plot(group["outer_iteration"], pd.to_numeric(group["unclipped_actor_surrogate_change"], errors="coerce"), linestyle=":", label=f"{method} unclipped")
        ax.set_title("Merit components")
        ax.set_xlabel("Outer iteration")
        ax.grid(alpha=0.3)
        ax.legend(fontsize=8, ncol=2)
        fig.tight_layout()
        ...
        (plots_dir / "stage6_merit_components.png").write_text("", encoding="utf-8")
    ...
    qp_actual = "proposed_qp_fullparam_mixed_clean_unclipped_actual_selector"
    qp_safe = "proposed_qp_fullparam_mixed_clean_unclipped_safe_minusG"
    qp_rarl = "proposed_qp_fullparam_mixed_rarl_unclipped_actual_selector"
    noG_full = "proposed_noG_fullparam_mixed_clean_unclipped"
    best_qp_training = max(final_metric(qp_actual, "final_training_return"), final_metric(qp_safe, "final_training_return"), final_metric(qp_rarl, "final_training_return"))
    best_qp_clean_stoch = max(final_metric(qp_actual, "clean_stoch_final"), final_metric(qp_safe, "clean_stoch_final"), final_metric(qp_rarl, "clean_stoch_final"))
    best_qp_clean_det = max(final_metric(qp_actual, "clean_det_final"), final_metric(qp_safe, "clean_det_final"), final_metric(qp_rarl, "clean_det_final"))
    best_qp_adv_stoch = max(final_metric(qp_actual, "control_adv_stoch_final"), final_metric(qp_safe, "control_adv_stoch_final"), final_metric(qp_rarl, "control_adv_stoch_final"))

    lines = [
        "# Stage 6 Full-Param Online Screen Report",
        "",
        ...
        "## Required Answers",
        "",
        f"1. Did full-param proposed update actor/log_std/critic? Check `stage6_fullparam_block_update_audit.csv` and `stage6_fullparam_qp_diagnostics.csv`.",
        f"2. Did QP beat noG under the same full-param return+cost merit? Compare `{qp_actual}` / `{qp_safe}` against `{noG_full}` in `stage6_fullparam_online_summary.csv`.",
        f"3. Did QP improve over Stage 5 actor-only proposed? Compare `stage5_online_summary.csv` vs `stage6_fullparam_online_summary.csv`.",
        f"4. Did QP beat SGD/EGM/PPM on training return? best QP=`{best_qp_training:.6g}`, SGD=`{final_metric('sgd', 'final_training_return'):.6g}`, EGM=`{final_metric('egm', 'final_training_return'):.6g}`, PPM=`{final_metric('ppm', 'final_training_return'):.6g}`.",
        f"5. Did QP beat SGD/EGM/PPM on clean stochastic? best QP=`{best_qp_clean_stoch:.6g}`, SGD=`{final_metric('sgd', 'clean_stoch_final'):.6g}`, EGM=`{final_metric('egm', 'clean_stoch_final'):.6g}`, PPM=`{final_metric('ppm', 'clean_stoch_final'):.6g}`.",
        f"6. Did QP beat SGD/EGM/PPM on clean deterministic? best QP=`{best_qp_clean_det:.6g}`, SGD=`{final_metric('sgd', 'clean_det_final'):.6g}`, EGM=`{final_metric('egm', 'clean_det_final'):.6g}`, PPM=`{final_metric('ppm', 'clean_det_final'):.6g}`.",
        f"7. Did QP beat SGD/EGM/PPM on control-adv stochastic? best QP=`{best_qp_adv_stoch:.6g}`, SGD=`{final_metric('sgd', 'control_adv_stoch_final'):.6g}`, EGM=`{final_metric('egm', 'control_adv_stoch_final'):.6g}`, PPM=`{final_metric('ppm', 'control_adv_stoch_final'):.6g}`.",
        f"8. mixed_clean vs mixed_rarl can now be compared directly via `{qp_actual}` / `{qp_safe}` / `{qp_rarl}` and their `short_clean_return_cost` / `short_rarl_return_cost` audits in `stage6_fullparam_online_summary.csv`.",
        f"9. Did gamma stay active online? Check `gamma_active_frac` for `{qp_actual}`, `{qp_safe}`, and `{qp_rarl}`.",
        f"10. Did fallback dominate or not? Check `fallback_to_noG_frac`.",
        f"11. Did value loss and log_std remain healthy? Check `value_loss_change`, `entropy_change`, `logstd_update_norm_mean`, `critic_update_norm_mean` in `stage6_fullparam_qp_diagnostics.csv`.",
        f"12. Is this line ready for longer run / multi-seed? Use final metrics plus diagnostic stability.",
        "",
        "## Notes",
        "",
        "- short_clean_return_cost and short_rarl_return_cost are final-checkpoint audits in this pass; protagonist-side online selectors use either mixed_clean_unclipped or mixed_rarl_unclipped depending on method.",
        "- Stochastic clean/control curves were computed post-hoc from saved checkpoints on the same eval schedule.",
        "- Adversary-side proposed updates stay in the same optimizer family, but no clean-return callback is injected there in this pass.",
        "",
    ]
    (output_root / "stage6_fullparam_report.md").write_text("\n".join(lines), encoding="utf-8")
