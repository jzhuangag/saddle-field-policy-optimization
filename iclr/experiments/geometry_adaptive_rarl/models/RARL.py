...
import torch
import numpy as np
from typing import Any, Callable, Dict, List, Iterable, Optional, Type, Union, Tuple
...
class _ZeroAdversaryPolicy:
    """Deterministic zero-force policy used only during clean protagonist warm-up."""
...
class RARL(BaseAlgorithm):
    ...
    def __init__(
        ...
        if protagonist: 
            ...
            self.protagonist = ALGOS_RARL[self.protagonist_algo](policy=self.protagonist_policy, env=env, n_steps=n_steps_protagonist, policy_kwargs=self.protagonist_policy_kwargs, verbose=self.verbose, tensorboard_log=tensorboard_log, seed=seed, device=self.device, **self.protagonist_kwargs)
        ...
        if adversary: 
            ...
            self.adversary = ALGOS_RARL[self.adversary_algo](policy=self.adversary_policy, env=adv_env, n_steps=n_steps_adversary, policy_kwargs=self.adversary_policy_kwargs, verbose=self.verbose, tensorboard_log=tensorboard_log, seed=seed, device=self.device, **self.adversary_kwargs)
    ...
    def learn(self,
            ...
            outer_correction: Optional[Callable[["RARL", int], None]] = None,
            callback: MaybeCallback = None,
        ...
        Args:
            ...
            outer_correction: Optional synchronized actor correction called once after both
                alternating PPO phases. The default ``None`` preserves standard RARL.
        ...
        """
        if update_mode not in {"alternating", "synchronized_lagged"}:
            raise ValueError(f"Unsupported RARL update_mode: {update_mode}")
        if update_mode == "synchronized_lagged" and (N_mu != 1 or N_nu != 1):
            raise ValueError("synchronized_lagged requires N_mu=N_nu=1")
        ...
        if outer_correction is not None and hasattr(outer_correction, "protagonist_callback"):
            callback_protagonist.append(outer_correction.protagonist_callback)
        if not callback_adversary:
            callback_adversary = []
        callback_adversary.append(SetupAdvTrainingCallback(policy=self.protagonist.policy,
                                                           verbose=self.verbose))

        lagged_protagonist_policy = None
        if update_mode == "synchronized_lagged":
            lagged_protagonist_policy = copy.deepcopy(self.protagonist.policy)
            lagged_protagonist_policy.set_training_mode(False)
        zero_adversary_policy = _ZeroAdversaryPolicy(int(np.prod(self.adversary.action_space.shape)))

        while iteration_i < total_timesteps:
            if lagged_protagonist_policy is not None:
                lagged_protagonist_policy.load_state_dict(self.protagonist.policy.state_dict())
                lagged_protagonist_policy.set_training_mode(False)
            if iteration_i > 0:
            ...
            if N_mu > 0:
                protagonist_opponent = self.adversary.policy
                if clean_warmup and iteration_i < max(adv_delay, 0):
                    protagonist_opponent = zero_adversary_policy
                for callback_item in callback_protagonist:
                    if hasattr(callback_item, "policy"):
                        callback_item.policy = protagonist_opponent
            ...
            if iteration_i >= adv_delay:
                ...
                if N_nu > 0:
                    adversary_opponent = (
                        lagged_protagonist_policy
                        if lagged_protagonist_policy is not None
                        else self.protagonist.policy
                    )
                    for callback_item in callback_adversary:
                        if hasattr(callback_item, "policy"):
                            callback_item.policy = adversary_opponent
            ...
            if outer_correction is not None and iteration_i >= max(adv_delay, 0):
                outer_correction(self, iteration_i)
